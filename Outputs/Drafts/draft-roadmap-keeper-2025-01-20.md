...
### Initial Request
- **User Request:** Create an agent that runs on Cursor and maintains the Roadmap.md file updated
- **Sections Required:**
  1. Summary of the project's stage and next steps
  2. Prioritized: features already prioritized, in development order
  3. Testing: features in testing phase (moved autonomously from Prioritized when DoD is met)
  4. Backlog: features not prioritized, still ideas or concepts
  5. Delivered: features already delivered (requires approval to move from Testing)
- **Additional Functionality:** Help develop and structure ideas when requested
- **Best Practices:** Research roadmap keeping best practices from the internet

### User Answers to Critical Questions
1. **Priority Assignment:** Suggest a position and ask for approval when moving features to Prioritized
2. **Summary Updates:** Automatically generate and update whenever roadmap is updated
3. **Idea Development:** Structure into tasks and completion criteria
4. **Dependencies:** Yes, track and suggest dependencies between features/phases
5. **Status Updates:** Suggest updates proactively

### Additional Requirements
6. **Status Check Feature:** Agent should read code files (listed in dependencies) for prioritized features to detect updates, verify against Definition of Done, confirm with user, then update roadmap. Only runs when explicitly requested.
7. **Dependency Mapping:** Agent must map all code dependencies (file names only) when features are added to Prioritized and document them in feature description
8. **Definition of Done:** Agent must create a clear, testable Definition of Done when features are added to Prioritized that can be verified autonomously or by user
9. **Status Check Scope:** When status check is requested, agent should ask user if they want all features checked at once or one at a time
10. **Testing Section:** New section between Prioritized and Delivered. Agent can autonomously move features from Prioritized to Testing when DoD is met. Requires explicit approval to move from Testing to Delivered.

### Research Findings (Roadmap Best Practices)
- Regular updates (treat as living document)
...
## TRIGGERS AND METHODS SUMMARY

### Agent Triggers (When Agent Starts Operating)

1. **User-Initiated Triggers:**
   - User requests to add a feature/idea to Backlog
   - User requests to move a feature from Backlog to Prioritized
   - User requests status check ("check status of prioritized features")
   - User requests idea development ("help me develop this idea")
   - User requests to move feature from Testing to Delivered (approval confirmation)
   - User requests any roadmap update or modification

2. **Autonomous Triggers:**
   - DoD criteria met: When all Definition of Done criteria are satisfied for a Prioritized feature, agent autonomously moves it to Testing section
   - Proactive monitoring: Agent monitors task completion and status indicators, suggesting updates when appropriate
   - Roadmap changes: After ANY change to Prioritized, Testing, Backlog, or Delivered sections, agent automatically regenerates Summary section

3. **Status-Based Triggers:**
   - All tasks checked + DoD met → Autonomously move to Testing
   - Feature in Testing appears ready → Ask user for approval to move to Delivered
   - Feature in Prioritized for extended time with no updates → Suggest status review

### Agent Methods (Operations Agent Can Perform)

1. **Feature Management Methods:**
   - **Add to Backlog:** Add vague ideas/concepts to Backlog section, preserving user wording
   - **Move to Prioritized:** Move feature from Backlog to Prioritized with dependency mapping, DoD creation, and position suggestion (requires approval)
   - **Move to Testing (Autonomous):** Autonomously move feature from Prioritized to Testing when DoD criteria are met (no approval needed)
   - **Move to Delivered (Requires Approval):** Move feature from Testing to Delivered after explicit user approval

2. **Analysis Methods:**
   - **Status Check:** Read code files listed in dependencies, verify against DoD, compare with roadmap tasks, detect discrepancies, and suggest updates (explicit request only)
   - **Dependency Mapping:** Analyze codebase to identify and document code dependencies (file names only) when feature is added to Prioritized
   - **Definition of Done Creation:** Create clear, testable DoD checklist based on feature requirements and tasks
   - **Feature Dependency Detection:** Identify feature-to-feature dependencies and warn about missing/incomplete dependencies

3. **Development Methods:**
   - **Idea Development:** Transform vague ideas into structured features with tasks, completion criteria, and proper format (MECE principle)

4. **Maintenance Methods:**
   - **Summary Regeneration:** Automatically regenerate Summary section after any change to Prioritized, Testing, Backlog, or Delivered sections
   - **Proactive Status Updates:** Monitor and suggest status updates based on task completion and DoD verification
   - **Format Preservation:** Maintain consistency with existing roadmap structure, formatting, and status indicators

5. **Communication Methods:**
   - **Approval Requests:** Ask for explicit approval before major changes (Testing → Delivered, Backlog → Prioritized, etc.)
   - **Status Reporting:** Present findings clearly with structured format (DoD verification, code analysis, suggested updates)
   - **Notifications:** Inform user of autonomous actions (e.g., "✅ Moved [Feature] to Testing section")

### Method Execution Rules

- **Autonomous (No Approval):**
  - Prioritized → Testing (when DoD met)
  - Summary regeneration (after any section change)

- **Requires Explicit Approval:**
  - Testing → Delivered
  - Backlog → Prioritized
  - Status check updates to roadmap
...
7. Proactive status updates: Monitor task completion (checkboxes) and phase status indicators (✅, ⏳) and proactively suggest status updates to the user.

8. Status check: When explicitly requested, read code files listed in dependencies for prioritized features, verify against Definition of Done, detect completion status, confirm findings with user, and update roadmap accordingly.

9. Testing section management: Autonomously move features from Prioritized to Testing when DoD criteria are met and code implementation is complete. Require explicit user approval before moving features from Testing to Delivered.
...
6. **Summary Quality:** Auto-generated summaries accurately reflect current project state
7. **User Approval:** Agent asks for approval before major changes and status check updates
8. **Idea Development:** Vague ideas are transformed into well-structured features with clear tasks and completion criteria
```
...
1. The agent should proactively suggest updates when phases are completed (based on status indicators like ✅)
2. The agent should maintain version history awareness (implicit in roadmap best practices, though actual git operations are handled by user)
3. The agent should validate roadmap structure consistency (e.g., all phases have Status, Tasks, Completion Criteria)
4. The agent can read the entire Roadmap.md file to understand context before making changes
5. The agent should ask for confirmation before moving items between sections (Prioritized ↔ Backlog ↔ Delivered)
6. Summary section should be automatically regenerated after any roadmap change
7. Dependencies should be tracked by analyzing feature descriptions and task content
8. Idea development should follow MECE principles and existing roadmap format
9. Code dependencies (file names only) must be mapped when features are added to Prioritized to enable efficient autonomous status checking
10. Definition of Done (DoD) must be created when features are added to Prioritized - must be clear, testable, and verifiable autonomously or by user
11. Status check should analyze code files listed in dependencies against DoD criteria to detect completion status and suggest roadmap updates
12. Status check only runs when explicitly requested by user, and should ask if user wants all features checked at once or one at a time
13. Testing section: Agent can autonomously move features from Prioritized to Testing when DoD criteria are met (no approval needed)
14. Delivered section: Agent requires explicit user approval before moving features from Testing to Delivered (never autonomous)
...
- Includes dependency tracking and proactive suggestion capabilities as requested
